...
## T & T - Trampoline {.tabset .tabset-fade}

```{r}
# read in data
t.NF.results <- read.csv(here::here("data","Trampoline-New_Flyers-Results-Tulips.csv")) %>%
    mutate(across(c(Team),factor))

t.IF.results <- read.csv(here::here("data","Trampoline-Intermediate_Flyers-Results-Tulips.csv")) %>%
    mutate(across(c(Team),factor))
...
```{r tramp.NF}
tramp.nf <- t.NF.results %>%
  select(Athlete, Team, Trampoline) %>%
  arrange(desc(Trampoline)) %>% 
  mutate(Rank = dense_rank(desc(Trampoline))) %>%
  relocate(Rank, Team)

datatable(tramp.nf, extensions = 'Buttons',
          options = list(dom = 'Bfrtip', buttons = c('copy', 'csv', 'excel', 'pdf', 'print')),
          rownames = FALSE)
```
...
```{r tramp.IF}
tramp.if <- t.IF.results %>%
  select(Athlete, Team, Trampoline) %>%
  arrange(desc(Trampoline)) %>% 
  mutate(Rank = dense_rank(desc(Trampoline))) %>%
  relocate(Rank, Team)

datatable(tramp.if, extensions = 'Buttons',
          options = list(dom = 'Bfrtip', buttons = c('copy', 'csv', 'excel', 'pdf', 'print')),
          rownames = FALSE)
```

## T & T - Double Mini {.tabset .tabset-fade}

```{r}
# read in data
dm.NF.results <- read.csv(here::here("data","Double_Mini-New_Flyers-Results-Tulips.csv")) %>%
    mutate(across(c(Team),factor))

dm.IF.results <- read.csv(here::here("data","Double_Mini-Intermediate_Flyers-Results-Tulips.csv")) %>%
    mutate(across(c(Team),factor))
...
```{r df.NF}
df.nf <- dm.NF.results %>%
  select(Athlete, Team, Double.Mini) %>%
  arrange(desc(Double.Mini)) %>% 
  mutate(Rank = dense_rank(desc(Double.Mini))) %>%
  relocate(Rank, Team)

datatable(df.nf, extensions = 'Buttons',
          options = list(dom = 'Bfrtip', buttons = c('copy', 'csv', 'excel', 'pdf', 'print')),
          rownames = FALSE)
```
...
datatable(df.if, extensions = 'Buttons',
          options = list(dom = 'Bfrtip', buttons = c('copy', 'csv', 'excel', 'pdf', 'print')),
          rownames = FALSE)
```


## Omnithon Results {.tabset .tabset-fade}

There were 5 gymnasts who competed in all 12 events offered!  

```{r df.OM}
# read in data
OM.results <- read.csv(here::here("data","Omnithon-results.csv")) %>%
    mutate(across(c(Team),factor))

OM.results <- OM.results %>%
  arrange(desc(Omnithon.Total)) %>% 
  mutate(Rank = dense_rank(desc(Omnithon.Total))) %>%
  relocate(Rank, Team)

datatable(OM.results, extensions = 'Buttons',
          options = list(dom = 'Bfrtip', buttons = c('copy', 'csv', 'excel', 'pdf', 'print')),
          rownames = FALSE)
```
